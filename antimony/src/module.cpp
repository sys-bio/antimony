...
string Module::AddUnitDefIfUnique(UnitDef* unitdef)
{
  for (size_t u=0; u<m_unitdefs.size(); u++) {
    UnitDef* match = m_unitdefs[u];
  ...
  m_unitdefs.push_back(unitdef);
  return unitdef->GetName();
}


bool Module::AddUnitDef(UnitDef* unitdef)
{
  for (size_t u=0; u<m_unitdefs.size(); u++) {
    UnitDef* match = m_unitdefs[u];
    if (match->GetName() == unitdef->GetName() && !unitdef->Matches(match)) {
      g_registry.SetError("Unit '" + match->GetName() + "' already defined differently; may not define it again.");
      return true;
    }
  }
  m_unitdefs.push_back(unitdef);
...
UnitDef* Module::GetUnitDef(std::string uid)
{
  for (size_t ud=0; ud<m_unitdefs.size(); ud++) {
    if (m_unitdefs[ud]->GetName() == uid) {
      return m_unitdefs[ud];
    }
  }
  return NULL;
}

size_t Module::GetNumUnitDefs()
{
  return m_unitdefs.size();
}

UnitDef* Module::GetUnitDef(size_t n)
{
  if (n >= m_unitdefs.size()) return NULL;
  return m_unitdefs[n];
}
